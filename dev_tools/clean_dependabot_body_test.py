...
import pytest
import os
from dev_tools.clean_dependabot_body import clean_content
...
def test_clean_content_basic(input_content):
    cleaned = clean_content(input_content)
    ...
    assert "Commits" in cleaned

def test_idempotency():
    # First pass
    cleaned = clean_content(SAMPLE_1)

    # Second pass - should return None as it is already cleaned
    cleaned_again = clean_content(cleaned)
    assert cleaned_again is None
